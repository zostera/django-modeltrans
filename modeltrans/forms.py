...
class TranslationModelFormMetaclass(forms.models.ModelFormMetaclass):
    def __new__(mcs, name, bases, attrs):
        ...
        1) fields - if fields are declared explicitly with the fields option,
           the related i18n fields will not be included, and these have to be added to base_fields.
        2) exclude - if fields are declared implicitly with the exclude option,
           the i18n translation fields, as well as the i18n field itself, will be included. We will remove the i18n field.
        ...
        i18n_field = get_i18n_field(opts.model)
        if not i18n_field:
            return new_class
        ...
        opts_fields = list(opts.fields) if opts.fields and opts.fields != ALL_FIELDS else None
        opts_exclude = list(opts.exclude) if opts.exclude else None

        languages = get_available_languages()

        for original_name in i18n_field.fields:  # for all translated fields

            # for all possible system languages
            for language in languages:
                field_name = build_localized_fieldname(original_name, language, ignore_default=True)

                # add i18n field if an explicitly chosen field
                if opts.fields and original_name in base_fields and field_name not in base_fields:
                    base_fields.append(field_name)
                    opts_fields.append(field_name)

                # remove field if an explicitly excluded field
                if opts.exclude and original_name in opts.exclude and field_name in base_fields:
            ...
            name = f"{original_name}_i18n"
            if name in base_fields:
                base_fields.remove(name)
            if opts.exclude:
                opts_exclude.append(name)

            # Remove the translation field for the system default language, because that already exists as the original field
            name = f"{original_name}_{get_default_language()}"
            if name in base_fields:
                base_fields.remove(name)
...
class TranslationModelForm(forms.ModelForm, metaclass=TranslationModelFormMetaclass):
    ...
    def get_included_fields(self):
        ...
        fields[ALL_FIELDS] = list(itertools.chain.from_iterable(fields.values()))
        return fields
    ...
    def order_translation_fields(self):
        ...
        if field_order:
            ...
            field_order = self.included_fields[ALL_FIELDS]
    ...
    def remove_excess_fields(self):
        ...
        for original_field_name in self.i18n_fields:
            for field in fields:
                if original_field_name in field and field not in self.included_fields[ALL_FIELDS]:
    ...
    def get_fallback_language(self, fallback_language=None):
        ...
        if (
            self.model_i18n_field
            and self.model_i18n_field.fallback_language_field
