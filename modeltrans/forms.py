...
class TranslationModelFormMetaClass(forms.models.ModelFormMetaclass):
    def __new__(mcs, name, bases, attrs):
        ...
        included_languages = get_available_languages()  # include all possible languages for the site
            ...
            if i18n_field:
                ...
                for original_field_name in i18n_field.fields:  # for all translated fields
                    ...
                    for language in included_languages:
                        field_name = build_localized_fieldname(original_field_name, language, ignore_default=True)

                        # add i18n field if an explicitly chosen field
                        if opts.fields and original_field_name in fields and field_name not in fields:
                            fields.append(field_name)
                            opts.fields.append(field_name)

                        # remove field if an explicitly excluded field
                        if opts.exclude and original_field_name in opts.exclude and field_name in fields:
...
class TranslationModelForm(forms.ModelForm, metaclass=TranslationModelFormMetaClass):
    ...
    - included_languages: a list defining languages for which translation fields are added.
        ...
        - The list order determines the order of fields in the form.  # TODO to make list order functional
    ...
    def included_fields(self):
        ...
        for original_field in self.form_i18n_fields:
            ...
            field_dict[original_field] = [
                build_localized_fieldname(original_field, language, ignore_default=True) for language in self.languages
            ]
    ...
    def set_included_field_properties(self):
        ...
        for original_field_name in self.form_i18n_fields:
            ...
            for language, field_name in zip(self.languages, self.included_fields[original_field_name]):
                ...
                self.fields[field_name].label = f"{original_field.label} ({language.upper()}, {label_text})"
    ...
    def remove_excess_fields(self):
        ...
        excluded_fields = [field for field in translation_fields if field not in self.included_fields["__all__"]]
    ...
    def get_fallback_language(self, fallback_language=None):
        ...
        if self.model_i18n_field.fallback_language_field and self.instance.pk:
            return get_instance_field_value(self.instance, self.model_i18n_field.fallback_language_field)
        ...
        if self.included_languages:
            ...
            for value in self.included_languages:
                ...
                if len(value) == 2 and value not in get_available_languages():
                    raise ValueError(f"included_languages: {value} is not an available language in the system")
                ...
                #    languages = get_available_languages()
                    ...
                    languages.append(value)  # assuming above checks assure this is an existing language in the system
